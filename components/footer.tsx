      ...
      <div className="max-w-[1200px] mx-auto px-6 pt-14 pb-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10">
        <div>
          <div className="flex items-center gap-2 mb-4">
            ...
            <span className="font-display text-base font-semibold text-cs-white tracking-[0.12em]">CASEOUT STUDIO</span>
          </div>
        ...
        <div>
          ...
          {[
            ...
            <Link key={p.href} href={p.href} className="block font-body text-[13px] text-cs-muted py-[5px] no-underline hover:text-cs-gold transition-colors">
              {p.label}
            </Link>
          ))}
        </div>
        <div>
          ...
          <p className="font-body text-[13px] text-cs-muted leading-8">
            Kopernika 30<br />Warszawa<br />hello@caseout.pl
          </p>
        </div>
        <div>
          <div className="font-mono text-[10px] text-cs-gold-dim tracking-[0.2em] mb-4">SOCIAL</div>
          {["Instagram", "YouTube", "SoundCloud", "Spotify"].map((s) => (
            <div key={s} className="font-body text-[13px] text-cs-muted py-[5px] cursor-pointer hover:text-cs-gold transition-colors">
              {s}
            </div>
          ))}
        </div>
